...
namespace SolarSpawn
{
	...
	{
		if (spawnedSolars.contains(pLaunch.iBase))
		{
			LaunchComm comm;
	...
	static void CreateFallbackBaseIfNeeded(const IObjRW* killedObject)
	{
		...
		if (solar->dockTargetId2 == 0)
			return;

		for (const auto& otherSolar : spawnedSolars)
		{
			if (otherSolar.first != solar->id && otherSolar.second.dockWith == solar->dockTargetId2)
				return;
		}

		fallbackBaseArchetype.baseId = solar->dockTargetId2;
	...
	void __stdcall SolarDestroyed(const IObjRW* killedObject, const bool killed, const uint killerShipId)
	{
		...
		for (const auto& entry : dockQueues[objId])
		{
			...
			if (GetShipInspect(shipId, inspect, starSystem) && inspect->cobj->ownerPlayer > 0 && inspect->is_invulnerable())
				pub::Player::ForceLand(inspect->cobj->ownerPlayer, static_cast<CEqObj*>(killedObject->cobj)->dockTargetId2);
		}
	...
	bool ExecuteCommandString(CCmds* cmds, const std::wstring& wscCmd)
	{
		if (IS_CMD("kill"))
		{
			...
			const uint clientId = HkGetClientIdFromCharname(cmds->GetAdminName());
		...
		if (IS_CMD("spawn"))
		{
			...
			cmds->Print(L"ERR solar not found: " + cmds->ArgStr(1));
			returncode = DEFAULT_RETURNCODE;
			return false;
		}
